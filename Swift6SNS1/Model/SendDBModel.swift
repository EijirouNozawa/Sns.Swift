    ...
    func sendData(roomNumber:String){
        ...
        imageRef.putData(contentImageData, metadata: nil) { (metadata, error) in
            ...
            imageRef.downloadURL { (url, error) in
                ...
                self.db.collection(roomNumber).document().setData(["userID":self.userID as Any,"userNmae":self.userName as Any,"comment":self.comment as Any,"userImage":self.userImageString as Any,"contentImage":url?.absoluteString as Any,"postDate":Date().timeIntervalSince1970])
